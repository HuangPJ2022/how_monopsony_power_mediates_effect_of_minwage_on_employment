...
base_cpi <- cpi_state[year == 1990, mean(cpi, na.rm = TRUE)] # base period: CPI value in 1990

cpi_state[, cpi_index := cpi / base_cpi * 100]         # rebase so 1990 = 100
...
# real wage in 2000 dollars
dt[, real_hourwage := avg_hourwage / cpi_index * 100]

# ==============================================================
# 4. Focus on the bottom 20% low-wage industries
# ==============================================================
# weighted average of variables of the bottom 20% low-wage industries(state x quarter)
...
modelsummary(est,
             ...
             title    = "Regression Results of Equation (2)")
...
p <- ggplot(df_coef, aes(x = j)) +
  ...
  labs(x = "Years relative to MW hike",
       ...
       title = "The Effect of Minimum Wage Increases on Log Average Real Wage") +
  theme_minimal(base_size = 12) +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(hjust = 0.5))

ggsave(file.path(outdir, "event_study_real_wage.png"), p,
       width = 8, height = 5, dpi = 300)

# ==============================================================
# 8. Pre-trend Test(hypothesis testing)
# ==============================================================
sink(file.path(outdir, "pretrend_tests.txt"))

cat("PRE-TREND TEST: D (H0: v_j jointly zero)\n")
cat("------------------------------------------\n")
pre_coefs <- paste0("j_annual::", c(-3, -2), ":D")
pre_coefs <- pre_coefs[pre_coefs %in% names(coef(est))]
if (length(pre_coefs) > 0) print(wald(est, pre_coefs))
